...
#Making the Pencil Sketch of Input Image
...
for i in range(num_rows):
    ...
    line = "".join([char_list[min(int(np.mean(image[row_pix:min_h, int(j*cell_w):min(int((j + 1) * cell_w), width)]) / 255 * num_chars), num_chars - 1)]for j in range(num_cols)]) + "\n"
...
out_image.save("./Output/PencilSketch/Output-4.jpg")